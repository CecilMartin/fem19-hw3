...
for i=1:1
    [coordinates,elements3,dirichlet,neumann] ...
        = refineRGB(coordinates,elements3,dirichlet,neumann,elem_marked);
end
...
[indicators_sorted, I] = sort(indicators,'descend');
elem_marked = I(1:ceil(0.1*length(indicators)));

% Record Data

error(end+1,:) = [length(elements3),sqrt(L2norm)];
indicator_rec{end+1} = indicators_sorted;
